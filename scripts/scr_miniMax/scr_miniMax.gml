...
function miniMax(p2_pieces){
	...
	*/
	...
	var lowestScorePlayer = [numberOfAIMoves]; //create an empty array of moves that are good for the AI to take 
	
	//now we will iterate through each of the AI boards and find the min for that board
	for(var i = 0; i < numberOfAIMoves; i++){
		
		var minValue = 20000; //set to high so we know we can fill in vlaue 
		
		for(var j = 0; j < numberOfHumanMoves; j++){

			//if we are at a lower value and we are seeing an entry associated with it 
			if(humanBoardArray[# j, 1] < minValue && humanBoardArray[# j, 4] == i){
				minValue = 	humanBoardArray[# j, 1];
			}
...
//assign the score to a given board based on the pieces that exist on it 
//Positive is good for the AI (player 2) and negative is advantage for the AI (player 2)
function scoreBoard(boardToScore){
	
	var countedPieces = 0;

	//summation of the board as a heuristic
	var boardSum = 0;
	
	//iterate through the board to look at each piece
	for(var i = 0; i < ds_grid_width(boardToScore) ; i++){
		for(var j = 0; j < ds_grid_height(boardToScore) ; j++){
			
			//skip the tiles that have nothing on it 
			if(boardToScore[# i, j] == noone){
				continue;	
			}
			countedPieces++;
			
			//If a piece is player 1 
			if(boardToScore[# i, j ].piecePlayer == 1){
				boardSum -= getPieceScore(boardToScore[# i, j].pieceType); //subtract value
			}else if(boardToScore[# i, j ].piecePlayer == 2){
				boardSum += getPieceScore(boardToScore[# i, j].pieceType); //increase value
			}else{
				show_debug_message("BIG ERROR BIG ERROR BIG ERROR")	
...
//Function to provide the heuristic weight of the provided piece type 
function getPieceScore(pieceType){
	
	//Set the weight of each piece
	var pawnWeight = 10;
	var knightWeight = 30;
	var bishopWeight = 30;
	var rookWeight = 50;
